...
export const getTrendingMovies = (mediaType = 'movie', timeWindow = 'day', page = 1) => {
  return fetch(
    `https://api.themoviedb.org/3/trending/${mediaType}/${timeWindow}?api_key=${import.meta.env.VITE_TMDB_KEY}&page=${page}`
  )
    .then((response) => {
      if (!response.ok) {
        return response.json().then((error) => {
...
export const searchMovies = (args) => {
  ...
  return fetch(
    `https://api.themoviedb.org/3/search/movie?api_key=${import.meta.env.VITE_TMDB_KEY}&language=en-US&query=${encodeURIComponent(query)}&page=1`
  )
    .then((response) => {
      if (!response.ok) {
        return response.json().then((error) => {
